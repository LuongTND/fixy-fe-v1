...
export default function ForgotPasswordPage() {
  ...
  const sendOtp = useCallback(async (values) => {
  ...
  const verifyOtp = useCallback(async () => {
    ...
    try {
      ...
      const nextError = err.response?.data?.message || 'Mã OTP không chính xác.';
      setError(nextError);
  ...
  const title = step === 1 ? 'Quên mật khẩu?' : step === 2 ? 'Xác thực OTP' : 'Đặt mật khẩu mới';
  ...
  return (
    <div className="w-full animate-fade-in-up font-montserrat">
      <div className="mb-8">
        <h2 className="mb-1 text-2xl font-bold leading-[30px] text-[#1b1c1c]">{title}</h2>
        <p className="text-sm leading-[21px] text-[#818A91]">{subtitle}</p>
      </div>

      {step === 1 && (
        <Form layout="vertical" requiredMark={false} onFinish={sendOtp} className="auth-ant-form">
          <Form.Item
            label="Email hoặc Số điện thoại"
            name="target"
            rules={[{ required: true, message: 'Vui lòng nhập Email hoặc Số điện thoại' }]}
          >
            <Input size="large" placeholder="example@email.com / 0901234567" autoComplete="username" />
          </Form.Item>
          <Button type="primary" htmlType="submit" loading={loading} block size="large" className="!h-11 !rounded !font-semibold">
            Gửi mã xác nhận
          </Button>
        </Form>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          {error && <Alert type="error" showIcon message={error} className="!mb-4 !rounded" />}
          ...
          <div className="mt-6 text-center">
            ...
            {resendTimer === 0 ? (
              <Button type="link" className="!h-auto !p-0 !font-semibold !text-primary" onClick={() => sendOtp()} disabled={loading}>
                Gửi lại mã
              </Button>
            ) : (
              <p className="text-[13px] text-[#9A9A9A]">Gửi lại sau {resendTimer}s</p>
            )}
          </div>

          <Button type="text" onClick={() => setStep(1)} className="mt-5 !w-full !font-semibold !text-[#818A91] hover:!text-primary">
      ...
      {step === 3 && (
        <Form layout="vertical" requiredMark={false} onFinish={resetPassword} className="auth-ant-form">
          <Form.Item
            label="Mật khẩu mới"
            name="newPassword"
            rules={[
          ...
          >
            <Input.Password size="large" placeholder="Ít nhất 8 ký tự" autoComplete="new-password" />
          </Form.Item>
      ...
      <div className="mt-8 text-center">
        ...
        <Link href="/login" className="text-sm font-semibold !text-primary no-underline">
          Đăng nhập ngay
        </Link>
      </div>
    </div>
