...
export default function DashboardPage() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const [summaryResult, trendsResult, servicesResult] =
      ...
      if (trendsResult.status === "fulfilled") {
        setBookingTrends(
          Array.isArray(trendsResult.value) ? trendsResult.value : [],
        );
      } else {
        setBookingTrends([]);
      }
      ...
      if (
        [summaryResult, trendsResult, servicesResult].some(
  ...
  const trendSummary = useMemo(() => {
    const completed = bookingTrends.reduce(
      (sum, item) => sum + Number(item.completedCount || 0),
      0,
    );
    const cancelled = bookingTrends.reduce(
      (sum, item) => sum + Number(item.cancelledCount || 0),
      0,
    );
  ...
  const categoryStats = useMemo(() => {
    const total = topServices.reduce(
      (sum, item) =>
        sum + Number(item.bookingCount || item.count || item.value || 0),
      0,
    );
    return topServices
      .map((item, index) => normalizeTopService(item, index, total))
      .slice(0, 5);
  }, [topServices]);
    ...
    <AdminShell activeKey="dashboard">
      <section className="admin-page-heading">
        <div>
          ...
          <p>
            Thống kê vận hành và hoạt động theo dữ liệu thực của hệ sinh thái
            Fixy.
          </p>
        </div>
      ...
      <Spin spinning={loading}>
        <section className="admin-kpi-grid">
          {stats.map((stat) => (
            <Card
              key={stat.label}
              className={`admin-kpi-card admin-kpi-${stat.tone}`}
            >
          ...
          <Card className="admin-panel admin-chart-panel">
            <div className="admin-panel-head">
              ...
              <div className="admin-trend-summary">
                <span>
                  <strong>{formatNumber(trendSummary.completed)}</strong> hoàn
                  thành
                </span>
                <span>
                  <strong>{formatNumber(trendSummary.cancelled)}</strong> đã hủy
                </span>
              </div>
            </div>
              ...
              {bookingTrends.length > 0 ? (
                <>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      ...
                      >
                        ...
                        <Tooltip
                          ...
                          cursor={{ fill: "rgba(255, 130, 40, 0.06)" }}
                        />
                        <Legend
                          verticalAlign="top"
                          align="right"
                  ...
                  {trendSummary.total === 0 && (
                    <div className="admin-trend-zero-state">
                      Chưa có đơn hoàn thành hoặc đã hủy trong khoảng thời gian
                      này.
                    </div>
                  )}
                </>
              ...
              <div className="admin-category-panel-content">
                <div className="admin-category-chart-container">
                  ...
                  <div className="admin-category-chart-center-label">
                    <p className="admin-category-center-name">
                      {activeCategoryName}
                    </p>
                    <p className="admin-category-center-value">
                  ...
                  {categoryStats.map((category, index) => (
                    ...
                    >
                      <div className="admin-category-item-info">
                        <span
                          className={`admin-category-bullet ${category.colorClass}`}
                        />
                        <span className="admin-category-name">
                          {category.name}
                        </span>
                        <strong className="admin-category-percent">
                          {category.value}%
                        </strong>
                      </div>
                    </div>
