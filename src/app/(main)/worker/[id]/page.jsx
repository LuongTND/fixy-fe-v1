...
export default function WorkerProfilePage({ params }) {
  ...
  const handleBookClick = () => {
    if (!worker?.id) return;
    if (!isAuthenticated) {
      message.warning("Vui lòng đăng nhập để đặt lịch với kỹ thuật viên.");
      router.push(`/login?redirect=${encodeURIComponent(`/worker/${id}`)}`);
      return;
    }
    router.push(`/booking/create?workerId=${worker.id}`);
  };
    ...
    async function loadWorker() {
      ...
      try {
        ...
        if (response?.id) {
          ...
          try {
            const reviewsResponse = await reviewApi.getWorkerReviews(
              response.id,
              {
                PageNumber: 1,
                PageSize: 10,
        ...
        if (alive) {
          message.error(
            error.response?.data?.message ||
              error.message ||
              "Không thể tải hồ sơ kỹ thuật viên",
          );
          setWorker(null);
        }
      } finally {
  ...
  const primaryService = useMemo(
    () =>
      worker?.services?.find((service) => service.isPrimary) ||
      worker?.services?.[0],
    [worker],
  );
  const serviceNames =
    worker?.services?.map((service) => service.categoryName).filter(Boolean) ||
    [];
  ...
  const tabs = [
    ...
    {
      id: "reviews",
      label: `Đánh giá (${reviewMeta?.totalCount ?? reviews.length})`,
    },
  ...
  if (!worker) {
    return (
      <div className="rounded-2xl border border-border-light bg-surface-bg p-10 text-center">
        ...
        <Link
          href="/search"
          className="font-body-bold text-primary no-underline"
        >
          Quay lại tìm kiếm
        </Link>
      </div>
  ...
  return (
    <div className="flex flex-col gap-8">
      <nav className="flex items-center gap-2 text-small text-text-muted">
        <Link
          href="/"
          className="text-text-muted no-underline transition-colors hover:text-primary"
        >
          Trang chủ
        </Link>
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-6">
              ...
              <div className="flex flex-1 flex-col gap-2">
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="m-0 font-h3 text-brand-navy">
                    {worker.fullName || "Kỹ thuật viên"}
                  </h1>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {serviceNames.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full bg-[#F5F5F5] px-3 py-1.5 font-small text-text-secondary"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className="rounded-2xl border border-border-light bg-surface-bg p-8 shadow-sm">
            {activeTab === "portfolio" && (
              <div className="flex flex-col gap-6">
                <div>
                  <h3 className="mb-3 font-body-bold text-brand-navy">
                    Giới thiệu
                  </h3>
                ...
                <div>
                  <h3 className="mb-4 font-body-bold text-brand-navy">
                    Hình ảnh công trình
                  </h3>
            ...
            {activeTab === "certificates" && (
              <div className="flex flex-col gap-4">
                <h3 className="m-0 font-body-bold text-brand-navy">
                  Bằng cấp & Chứng chỉ
                </h3>
            ...
            {activeTab === "schedule" && (
              <div className="flex flex-col gap-6">
                    ...
                    {DAY_META.map((day) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {isActive
                              ? `${trimTime(row.startTime)} - ${trimTime(row.endTime)}`
                              : "Nghỉ"}
                          </div>
                        </div>
                      );
                ...
                <div className="rounded-xl border border-border-light bg-surface-bg p-4">
                  <h4 className="m-0 mb-3 font-body-bold text-brand-navy">
                    Ngày nghỉ đặc biệt
                  </h4>
                  {exceptions.length ? (
                    <div className="flex flex-col gap-2">
                      {exceptions.map((item) => (
                        <div
                          ...
                          className="flex flex-col gap-1 rounded-lg bg-error/5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"
                        >
                ...
                {loadingReviews ? (
                  ...
                  <ul className="m-0 flex list-none flex-col gap-4 p-0">
                    {reviews.map((review) => {
                      ...
                      return (
                        <li
                          key={review.id || review.createdDate}
                          className="rounded-xl border border-border-light bg-[#F9F9F9] p-4"
                        >
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                              ...
                              <p className="m-0 mt-1 text-small text-text-muted">
                                {formatDate(
                                  review.createdDate || review.createdAt,
                                )}
                              </p>
                            </div>
                          ...
                          {review.comment && (
                            <p className="m-0 mt-3 text-small leading-relaxed text-text-secondary">
                              {review.comment}
                            </p>
                          )}
                  ...
                  <div className="rounded-xl border border-dashed border-border-light bg-[#F9F9F9] p-8 text-center text-text-tertiary">
                    <span className="material-symbols-outlined mb-3 block text-[44px] text-border-medium">
                      rate_review
                    </span>
        ...
        <div>
          <div className="sticky top-24 flex flex-col gap-5 rounded-2xl border border-border-light bg-surface-bg p-6 shadow-md">
            <div>
              ...
              <p className="m-0 mt-1 font-h3 text-primary">
            ...
            <div className="flex flex-col gap-2.5">
              <button
                ...
                className="w-full cursor-pointer rounded-xl border-none bg-primary py-4 font-body-bold text-white shadow-md transition-all hover:brightness-105 active:scale-[0.98]"
              >
                Đặt lịch ngay
              </button>
