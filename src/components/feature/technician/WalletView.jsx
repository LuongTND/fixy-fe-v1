...
const transactionTypes = {
  bookingincome: { label: "Thu nhập từ đơn hàng", icon: "payments" },
  refund: { label: "Hoàn tiền", icon: "restart_alt" },
  withdrawal: { label: "Rút tiền", icon: "local_atm" },
};

const getTxType = (type) => String(type || "").toLowerCase();
...
export function WalletView() {
  ...
  const selectedWithdrawAccount = useMemo(
    () =>
      accounts.find((account) => account.id === withdrawForm.payoutAccountId) ||
      null,
  ...
  const loadData = useCallback(
    async (pageNumber = meta.pageNumber, pageSize = meta.pageSize) => {
      setLoading(true);
      try {
        const [walletResult, accountsResult, payoutsResult] =
        ...
        if (accountsResult.status === "fulfilled") {
          ...
          setAccounts([]);
          setAccountsError(
            "Không thể tải tài khoản nhận tiền. Bạn vẫn có thể thêm tài khoản mới hoặc thử lại sau.",
          );
        }
      } catch (error) {
        message.error(
  ...
  const handleCreateAccount = async (event) => {
    ...
    if (
      ...
      message.warning(
        "Vui lòng nhập đầy đủ số tài khoản, tên chủ tài khoản và ngân hàng.",
      );
      return;
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "right",
      render: (value) => (
        <span className="font-bold text-[#FF8228]">
    ...
    },
    {
      ...
      render: (value) => {
        const status =
          payoutStatus[getStatusKey(value)] || payoutStatus.pending;
  ...
  const transactionColumns = [
    {
      ...
      key: "transaction",
      render: (_, record) => {
        const typeKey = getTxType(record.type);
        const config = transactionTypes[typeKey] || {
          label: record.type || "Giao dịch ví",
        ...
        return (
          <div className="flex items-center gap-3 font-montserrat">
            ...
            <div>
              <p className="m-0 text-sm font-bold text-on-surface leading-tight">
                {config.label}
              </p>
              <p className="m-0 text-[11px] text-text-muted leading-none mt-1">
                Mã: {record.id ? `${record.id.slice(0, 8)}...` : "-"}
              </p>
            </div>
          </div>
        );
    ...
    },
    {
      ...
      render: (_, record) => {
        const isDebit = String(record.direction).toLowerCase() === "debit";
        const sign = isDebit ? "-" : "+";
        const color = isDebit ? "#EA4335" : "#39B54A";
        return (
    ...
    <main className="mx-auto max-w-[1400px] p-md md:p-lg">
      <div className="mb-lg flex flex-col justify-between gap-sm md:flex-row md:items-end">
        <div>
          ...
          <p className="text-sm text-text-secondary">
            Quản lý thu nhập, tài khoản nhận tiền và yêu cầu rút tiền.
          </p>
        </div>
      ...
      <div className="mb-lg grid grid-cols-1 gap-md md:grid-cols-3">
        <div className="rounded-xl border border-border-light bg-gradient-to-br from-primary-container to-[#9a4600] p-lg text-white shadow-md">
          ...
          <h2 className="mt-2 text-[36px] font-bold">
            {loading ? "..." : formatCurrency(wallet?.balance)}
          </h2>
          <p className="mt-3 text-xs opacity-80">
            Có thể tạo yêu cầu rút về tài khoản ngân hàng đã lưu.
          </p>
        </div>
        <div className="rounded-xl border border-border-light bg-white p-md shadow-sm">
          ...
          <h3 className="mt-2 font-h3">
            {loading ? "..." : formatCurrency(wallet?.lifetimeEarned)}
          </h3>
        </div>
        <div className="rounded-xl border border-border-light bg-white p-md shadow-sm">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-text-muted">
          ...
          <h3 className="mt-2 font-h3">
            {loading ? "..." : formatCurrency(wallet?.lifetimeSpent)}
          </h3>
        </div>
      </div>
      ...
      <div className="mb-lg grid grid-cols-1 gap-lg xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
        <div className="flex flex-col gap-lg min-w-0">
          <section className="rounded-xl border border-border-light bg-white p-md shadow-sm animate-fade-in-up">
            <div className="mb-md flex items-center justify-between">
              <h2 className="font-body-bold">Yêu cầu rút tiền</h2>
              <Button
                size="small"
                onClick={() => loadData(meta.pageNumber, meta.pageSize)}
              >
                Làm mới
              </Button>
            </div>
            <Table
              ...
              loading={loading}
              locale={{
                emptyText: <Empty description="Chưa có yêu cầu rút tiền" />,
              }}
              pagination={{
          ...
          <section className="rounded-xl border border-border-light bg-white p-md shadow-sm">
            <div className="mb-md flex items-center justify-between">
              <h2 className="font-body-bold">Lịch sử giao dịch</h2>
              <Button
                size="small"
                onClick={() => loadData(meta.pageNumber, meta.pageSize)}
              >
            ...
            <Table
              rowKey={(record) => record.id || record.createdDate}
              columns={transactionColumns}
              dataSource={wallet?.recentTransactions || []}
              loading={loading}
              locale={{
        ...
        <section className="rounded-xl border border-border-light bg-white p-md shadow-sm">
          ...
          <div className="space-y-sm">
            {accounts.length === 0 && (
              <Empty description="Chưa có tài khoản nhận tiền" />
            )}
            {accounts.map((account) => (
              <div
                key={account.id}
                className="rounded-xl border border-border-light p-sm"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="m-0 text-xs font-medium text-text-muted">
                Chọn ngân hàng từ danh sách VietQR hỗ trợ
              </p>
            </div>
          </div>
        }
            ...
            <Select
              ...
              onChange={(value) => {
                const bank = banks.find(
                  (item) => item.code === value || item.bin === value,
                );
              ...
              optionRender={(option) => (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="m-0 truncate text-sm font-bold text-on-surface">
        ...
        title={
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="m-0 text-xs font-medium text-text-muted">
                Chọn tài khoản nhận tiền và nhập số tiền cần rút
              </p>
            </div>
          </div>
        }
        ...
        <form className="mt-6 space-y-5" onSubmit={handleCreatePayout}>
          ...
          {selectedWithdrawAccount && (
            <div className="rounded-xl border border-border-light bg-[#FBF9F8] p-sm">
              ...
              <div className="mt-2 flex items-start justify-between gap-3">
                <div>
                  <p className="font-bold text-on-surface">
                    {selectedWithdrawAccount.bankName}
                  </p>
                  <p className="text-sm text-text-secondary">
                    {selectedWithdrawAccount.accountName}
                  </p>
                  <p className="text-xs text-text-muted">
                    {selectedWithdrawAccount.bankCode || "Bank code"} ·{" "}
                    {maskAccount(selectedWithdrawAccount.accountNumber)}
                  </p>
                </div>
                {selectedWithdrawAccount.isDefault && (
          ...
          <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-border-light bg-white">
            ...
            <div className="p-sm">
              ...
              <p
                className={`mt-1 font-bold ${remainingBalance < 0 ? "text-error" : "text-on-surface"}`}
              >
