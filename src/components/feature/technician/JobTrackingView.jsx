...
export function JobTrackingView({ bookingId }) {
  ...
  const handleOpenChat = () => {
    if (statusKey === "pending" || statusKey === "matching") {
      message.warning(
        "Vui lòng chấp nhận yêu cầu đặt lịch để có thể nhắn tin.",
      );
      return;
    }
    setChatOpen(true);
  ...
  const handleProposeJob = async () => {
    ...
    try {
      ...
      message.error(
        error.response?.data?.message ||
          error.message ||
          "Không thể gửi đề xuất thương lượng.",
      );
    } finally {
      setTransitioning(false);
    }
  ...
  const handleUploadPhotos = async (e) => {
    ...
    try {
      ...
      const newIds = mediaList
        .map((item) => item.id || item.mediaId || item.fileId)
        .filter(Boolean);
      const newUrls = mediaList
        .map((item) => item.fileUrl || item.url)
        .filter(Boolean);
  ...
  const handleCompleteBooking = async () => {
    if (uploadedMediaIds.length === 0) {
      message.warning(
        "Vui lòng tải lên ít nhất 1 ảnh nghiệm thu trước khi hoàn thành.",
      );
      return;
    }
    setTransitioning(true);
  ...
  const isProposed =
    booking?.workerProposedPrice && booking?.workerProposedPrice > 0;
  ...
  const customerAvatar =
    booking?.customer?.avatar || booking?.user?.avatarUrl || "";

  const totalAmount =
    booking?.totalAmount || booking?.finalPrice || booking?.estimatedPrice || 0;
  ...
  const requestImages =
    Array.isArray(booking?.requestImages) && booking.requestImages.length > 0
      ...
      : legacyMedia.filter(
          (med) =>
            med.category !== MEDIA_CATEGORY.COMPLETION && med.category !== 4,
        );
  const completeImages =
    Array.isArray(booking?.completeImages) && booking.completeImages.length > 0
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (
          elapsed < WORKER_LOCATION_MIN_INTERVAL_MS &&
          moved < WORKER_LOCATION_MIN_DISTANCE_M
        ) {
          return;
        }
    ...
    return (
      <div className="mx-auto max-w-[800px] py-12 px-4 font-montserrat text-center">
        <div className="rounded-2xl border-2 border-dashed border-[#dec0b1]/30 bg-white p-10 shadow-sm">
          <span className="material-symbols-outlined text-5xl text-[#818A91] mb-3">
            warning
          </span>
          ...
          <p className="text-sm text-[#818A91]">
            Đơn hàng này không tồn tại hoặc bạn không được phân bổ thực hiện.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <main className="max-w-[1200px] mx-auto px-4 py-8 font-montserrat text-[#1b1c1c] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-extrabold tracking-tight">
            Chi tiết công việc #
            {booking.code || booking.id.substring(0, 8).toUpperCase()}
          </h1>
          <p className="text-xs font-semibold text-[#818A91] mt-1 flex items-center gap-1">
            <span className="material-symbols-outlined text-[16px] text-[#FF8228]">
              calendar_today
            </span>
            Thời gian hẹn:{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {[
            ...
            <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h2 className="text-xs font-bold uppercase tracking-wider text-[#818A91] mb-6">
                Tiến trình dịch vụ
              </h2>
              <div className="relative">
                <div className="absolute left-[19px] top-0 bottom-0 w-1 bg-slate-100 hidden md:block" />
                <div
                  className={`absolute left-[19px] top-0 hidden w-1 bg-[#39B54A] transition-all duration-500 md:block ${stepProgressClass}`}
                />
                ...
                <div className="space-y-6 relative">
                  {STEP_CONFIG.map((step, i) => {
                    ...
                    return (
                      <div
                        key={step.key}
                        className={`flex items-start gap-4 ${!done && !active ? "opacity-40" : ""}`}
                      >
                        <div
                          className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm ${
                              ...
                              : active
                                ? "bg-white border-4 border-[#FF8228]"
                        ...
                        >
                          {done ? (
                            ...
                            <div className="w-2.5 h-2.5 rounded-full bg-[#FF8228] animate-pulse" />
                          ) : (
                        ...
                        <div className="pt-1.5 flex-1">
                          <h3
                            className={`text-xs font-bold uppercase tracking-wider ${done ? "text-[#39B54A]" : active ? "text-[#FF8228]" : "text-[#818A91]"}`}
                          >
                            {step.label}
                          </h3>
                          {done && (
                            <p className="text-[11px] text-[#39B54A] mt-0.5 font-medium">
                              Đã hoàn thành
                            </p>
                          )}
                          {active && (
                            <p className="text-[11px] text-[#818A91] mt-0.5 font-medium">
                              Đang xử lý...
                            </p>
                          )}
                        </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap gap-3">
                {statusKey === "confirmed" && (
                  <button
                    ...
                    className="px-6 py-2.5 bg-[#FF8228] text-white rounded-xl font-bold text-xs hover:brightness-105 transition-all flex items-center gap-2 shadow-sm"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      directions_car
                    </span>
                ...
                {statusKey === "inprogress" && (
                  <button
                    ...
                    disabled={transitioning}
                    className="px-6 py-2.5 bg-[#39B54A] text-white rounded-xl font-bold text-xs hover:brightness-105 transition-all flex items-center gap-2 shadow-sm"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      verified
                    </span>
                ...
                {statusKey === "completed" && (
                  <div className="flex items-center gap-2 text-[#39B54A]">
                    ...
                    <span className="font-bold text-sm">
                      Công việc đã hoàn thành và bàn giao!
                    </span>
                  </div>
                )}
              </div>
          ...
          {statusKey === "pending" && (
            <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h2 className="text-base font-bold mb-3">
                Phản hồi yêu cầu công việc
              </h2>
              <p className="text-xs text-[#818A91] mb-6">
                Bạn có thể chấp nhận công việc theo điều kiện gốc, đề xuất thay
                đổi hoặc từ chối yêu cầu.
              </p>

              {isProposed ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 mb-6">
                  <h4 className="text-xs font-bold text-[#FF8228] uppercase tracking-wider">
                    Đề xuất thương lượng đã gửi
                  </h4>
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium">
                    <div>
                      ...
                      <span className="block font-bold text-sm text-[#FF8228] mt-0.5">
                        {formatCurrency(booking.workerProposedPrice)}
                      </span>
                    </div>
                    <div>
                      <span className="text-[#818A91]">
                        Thời gian bạn đề xuất:
                      </span>
                      <span className="block font-bold text-[#1b1c1c] mt-0.5">
                        {formatDate(booking.workerProposedTime)}
                      </span>
                    </div>
                  ...
                  <p className="text-[10px] text-[#818A91] mt-4 font-bold uppercase tracking-wider">
                    Đang chờ khách hàng đồng ý hoặc phản hồi tin nhắn...
                  </p>
                </div>
              ) : (
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    disabled={transitioning}
                    className="px-5 py-2.5 bg-[#39B54A] text-white rounded-xl font-bold text-xs hover:brightness-105 transition-all flex items-center gap-1.5 shadow-sm"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      check
                    </span>
                  ...
                  <button
                    ...
                    disabled={transitioning}
                    className="px-5 py-2.5 border-2 border-[#FF8228] text-[#FF8228] bg-white rounded-xl font-bold text-xs hover:bg-orange-50 transition-all flex items-center gap-1.5"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      handshake
                    </span>
                  ...
                  <button
                    ...
                    disabled={transitioning}
                    className="px-5 py-2.5 border border-slate-200 text-rose-500 bg-white rounded-xl font-bold text-xs hover:bg-slate-50 transition-all flex items-center gap-1.5"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      close
                    </span>
          ...
          {statusKey === "pendingpayment" && (
            <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm text-center py-10">
              ...
              <p className="text-xs text-[#818A91] mt-1 max-w-[400px] mx-auto leading-relaxed">
                Khách hàng đã chấp nhận điều kiện dịch vụ và đang tiến hành
                thanh toán. Hệ thống sẽ tự động thông báo khi đơn được xác nhận.
              </p>
              <button
                type="button"
                onClick={() =>
                  openSupportTicket({
                    ...
                    contextLabel:
                      "Dành cho lỗi thanh toán, đối soát hoặc chưa ghi nhận tiền của công việc này.",
                  })
                }
          ...
          {["inprogress", "completed"].includes(statusKey) && (
            <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <div className="flex justify-between items-center mb-5">
                <h2 className="text-xs font-bold uppercase tracking-wider text-[#818A91]">
                  Ảnh nghiệm thu công việc
                </h2>
                <span className="px-2 py-0.5 bg-[#FF8228]/10 text-[#FF8228] rounded-md text-[10px] font-bold uppercase tracking-wider">
                  Yêu cầu ảnh chụp
                </span>
              </div>
              ...
              {statusKey === "inprogress" && (
                <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100/60 transition-all cursor-pointer mb-5 relative">
                  ...
                  />
                  <span className="material-symbols-outlined text-slate-400 mb-2 text-[36px]">
                    add_a_photo
                  </span>
                  <p className="text-xs font-bold text-[#1b1c1c]">
                    Chọn hoặc chụp ảnh nghiệm thu
                  </p>
                  <p className="text-[10px] text-[#818A91] mt-1 max-w-[280px] text-center">
                    Bao gồm thiết bị đã được sửa chữa hoặc biên bản nghiệm thu
                    (tối đa 5 ảnh)
                  </p>
                  {uploadingPhotos && (
                    <span className="text-xs text-[#FF8228] font-bold mt-2">
                      Đang tải ảnh lên...
                    </span>
                  )}
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 sm:grid-cols-5">
                {uploadedPhotoUrls.map((url, i) => (
                  <div
                    key={i}
                    className="group relative aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-100 [&_.ant-image-img]:!h-full [&_.ant-image-img]:!w-full [&_.ant-image-img]:!object-cover [&_.ant-image-mask]:!flex [&_.ant-image-mask]:!items-center [&_.ant-image-mask]:!justify-center [&_.ant-image]:!h-full [&_.ant-image]:!w-full"
                  >
                    <AntImage
                      src={url}
                ...
                {statusKey === "completed" &&
                  ...
                  completeImages.map((med) => (
                    <div
                      key={med.id || med.fileUrl}
                      className="group relative aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-100 [&_.ant-image-img]:!h-full [&_.ant-image-img]:!w-full [&_.ant-image-img]:!object-cover [&_.ant-image-mask]:!flex [&_.ant-image-mask]:!items-center [&_.ant-image-mask]:!justify-center [&_.ant-image]:!h-full [&_.ant-image]:!w-full"
                    >
                      <AntImage
          ...
          {statusKey === "completed" && (
            <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  <h2 className="m-0 text-xs font-bold uppercase tracking-wider text-[#818A91]">
                    Đánh giá từ khách hàng
                  </h2>
                  <p className="m-0 mt-1 text-xs text-[#818A91]">
                    Phản hồi lại đánh giá sau khi hoàn thành dịch vụ.
                  </p>
                </div>
                <span className="material-symbols-outlined text-[#FF8228]">
                  rate_review
                </span>
              </div>

              {loadingReview ? (
                <div className="rounded-xl bg-slate-50 p-4 text-center text-xs font-semibold text-[#818A91]">
                  Đang tải đánh giá...
                </div>
              ) : bookingReview ? (
                <div className="space-y-4">
                  <div className="rounded-xl border border-[#dec0b1]/20 bg-[#FFF8F3] p-4">
                    <div className="mb-3 flex items-start justify-between gap-3">
                      <div>
                        <p className="m-0 text-sm font-extrabold text-[#1b1c1c]">
                          {bookingReview.customer?.fullName || customerName}
                        </p>
                        <p className="m-0 mt-1 text-xs text-[#818A91]">
                          {formatDate(
                            bookingReview.createdDate ||
                      ...
                      <div className="flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-[#FF8228] shadow-sm">
                        ...
                        <span className="text-xs font-black">
                          {Number(bookingReview.rating || 0).toFixed(1)}
                        </span>
                      </div>
                    </div>
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-5">
                          {bookingReview.images.map((image) => {
                            ...
                            return (
                              <div
                                key={image.id || url}
                                className="group aspect-square overflow-hidden rounded-lg border border-white bg-slate-50 [&_.ant-image-img]:!h-full [&_.ant-image-img]:!w-full [&_.ant-image-img]:!object-cover [&_.ant-image-mask]:!flex [&_.ant-image-mask]:!items-center [&_.ant-image-mask]:!justify-center [&_.ant-image]:!h-full [&_.ant-image]:!w-full"
                              >
                                <AntImage
                                  src={url}
                                  alt="Ảnh đánh giá"
                  ...
                  {bookingReview.workerReply ? (
                    <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                      <p className="m-0 text-xs font-bold uppercase tracking-wider text-emerald-600">
                        Phản hồi của bạn
                      </p>
                      <p className="m-0 mt-2 text-sm leading-relaxed text-[#1b1c1c]">
                        {bookingReview.workerReply}
                      </p>
                      {(bookingReview.repliedAt ||
                        bookingReview.repliedDate) && (
                        <p className="m-0 mt-2 text-[11px] text-[#818A91]">
                          {formatDate(
                            bookingReview.repliedAt ||
                              bookingReview.repliedDate,
                          )}
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                      <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#818A91]">
                        Phản hồi đánh giá
                      </label>
                      <textarea
                        value={replyText}
          ...
          {/* Job summary details */}
          <section className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#818A91] mb-4">
              Thông tin công việc chi tiết
            </h2>

            <div className="space-y-4">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#818A91] block">
                  Mô tả từ khách hàng
                </span>
                <p className="text-sm font-semibold text-[#4A4A4A] mt-1 leading-relaxed whitespace-pre-line">
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <p className="text-sm font-bold text-[#1b1c1c] mt-1">
            ...
            {requestImages.length > 0 && (
              <div className="mt-5 pt-5 border-t border-slate-100">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#818A91] block mb-3">
                  Hình ảnh khách đính kèm
                </span>
                <AntImage.PreviewGroup>
                  <div className="grid grid-cols-3 gap-2.5 sm:grid-cols-5">
                    {requestImages.map((med) => (
                      <div
                        key={med.id || med.fileUrl}
                        className="group aspect-square overflow-hidden rounded-xl border border-slate-200 bg-[#F5F5F5] [&_.ant-image-img]:!h-full [&_.ant-image-img]:!w-full [&_.ant-image-img]:!object-cover [&_.ant-image-mask]:!flex [&_.ant-image-mask]:!items-center [&_.ant-image-mask]:!justify-center [&_.ant-image]:!h-full [&_.ant-image]:!w-full"
                      >
                        <AntImage
                          src={med.fileUrl || med.url}
                          alt="Ảnh sự cố"
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#FF8228] p-5 flex items-center gap-4">
              <div className="w-14 h-14 rounded-full border-[3px] border-white overflow-hidden shrink-0 bg-slate-200">
                <img
                  src={
                    customerAvatar ||
                    "https://lh3.googleusercontent.com/aida-public/AB6AXuC6BDEHEz7H6UzRq5va4XltTBkQl6L_LC4FNAHi2Ak5vGY72Qvl64jJOJNFb1gPziJN0ujIQT2M-ouDjr9TTOEarYTqKbTuI9ZO4TgjTeG_ZK7ZDWzfKaQIyF2U8Qr1ry_Pb98eZHu4ViEQ024J0TQyMltzfn6P_RcCf_QjwNr98rK4cPSVM2nrEQVA8aqJ4k3K5nosdsW4B3QsTNm78rHm_0dPkdxTRDDM0taG5PBjpKXrsTQUp6iFaK-lXwD2KAVGuV2s5S-MBJ8"
                  }
                  alt="Khách hàng"
            ...
            <div className="p-5 space-y-4">
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() =>
                    message.info("Tính năng gọi ẩn danh đang được phát triển.")
                  }
                ...
                <button
                  ...
                  title={
                    statusKey === "pending" || statusKey === "matching"
                      ? "Vui lòng chấp nhận yêu cầu để nhắn tin"
                      : ""
                  }
                >
                  <span className="material-symbols-outlined text-[20px]">
              ...
              <div className="pt-4 border-t border-slate-100 text-xs">
                <span className="text-[#818A91] font-bold uppercase tracking-wider block mb-1">
          ...
          <section className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
            <h3 className="text-xs font-bold uppercase tracking-wider text-[#818A91] mb-4">
              Chi tiết thu nhập
            </h3>
            <div className="space-y-3.5 text-xs font-semibold text-slate-700">
              <div className="flex justify-between">
                ...
                <span>
                  {formatCurrency(booking.basePrice || booking.estimatedPrice)}
                </span>
              </div>
              {isProposed && (
              ...
              <div className="pt-3 border-t border-slate-100 flex justify-between items-center text-sm">
                ...
                <span className="font-black text-lg text-[#FF8228]">
      ...
      {proposalModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-xl w-full max-w-[450px] p-6 animate-fade-in"
          ...
          >
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-[#818A91] mb-1.5">
                  Giá đề xuất mới (VNĐ)
                </label>
                <input
                  type="number"
            ...
            <div className="flex gap-3 justify-end text-xs">
              <button
                onClick={() => setProposalModal(false)}
                className="px-5 py-2.5 border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50"
              >
                Hủy
              </button>
              <button
                ...
                disabled={transitioning}
                className="px-5 py-2.5 bg-[#FF8228] text-white rounded-xl font-bold hover:brightness-105 transition-all"
              >
                Gửi đề xuất
              </button>
            </div>
      ...
      {rejectModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-xl w-full max-w-[420px] p-6 animate-fade-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-base text-rose-500">
                Từ chối công việc
              </h3>
              <button
                onClick={() => setRejectModal(false)}
            ...
            <div className="flex gap-3 justify-end text-xs">
              ...
              <button
                ...
                disabled={transitioning}
                className="px-5 py-2.5 bg-rose-500 text-white rounded-xl font-bold hover:brightness-105 transition-all"
              >
                Xác nhận từ chối
              </button>
            </div>
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#818A91] mt-0.5">
                    Đặt lịch #{bookingId.substring(0, 8).toUpperCase()}
                  </p>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div
                      className={`p-3 rounded-2xl shadow-sm text-xs leading-relaxed ${
                        isMe
                          ? "bg-[#FF8228] text-white rounded-br-none"
                    ...
                    >
                      ...
                      <span
                        className={`block text-[8px] mt-1 text-right ${isMe ? "text-white/70" : "text-[#818A91]"}`}
                      >
                        {parseBackendDate(
              ...
              <div className="p-3 bg-white border-t border-slate-100 shrink-0">
                ...
                >
                  <div className="flex shrink-0">
                    <label
                      className="p-2 text-[#FF8228] hover:bg-orange-50 rounded-full cursor-pointer transition-colors"
                      title="Gửi ảnh"
                    >
                      <span className="material-symbols-outlined text-[20px]">
                        image
                  ...
                  <div className="flex-1 relative">
                    ...
                    <button
                      ...
                      className="absolute right-2.5 bottom-2 text-[#FF8228] disabled:opacity-50 transition-transform active:scale-95"
                    >
                      <span className="material-symbols-outlined text-[20px] material-symbols-filled">
                        send
                      </span>
