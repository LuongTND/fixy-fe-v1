...
const statusOptions = [
  {
    ...
    key: "open",
    label: "Mở",
    className: "bg-[#FFF4ED] text-[#FF8228] border-[#FFD9C2]",
  },
  {
    value: SUPPORT_STATUS.IN_PROGRESS,
    key: "inprogress",
    label: "Đang xử lý",
    className: "bg-[#E8F8FF] text-[#009BD6] border-[#B9EBFF]",
  },
  {
    value: SUPPORT_STATUS.RESOLVED,
    key: "resolved",
    label: "Đã giải quyết",
    className: "bg-[#EFFFF6] text-[#23A455] border-[#BDEFD2]",
  },
  {
    value: SUPPORT_STATUS.CLOSED,
    key: "closed",
    label: "Đã đóng",
    className: "bg-[#F5F5F5] text-[#555555] border-[#DDDDDD]",
  },
];

const categoryOptions = [
  {
    value: SUPPORT_CATEGORY.DISPUTE,
    key: "dispute",
    label: "Tranh chấp",
    icon: "gavel",
  },
  {
    value: SUPPORT_CATEGORY.PAYMENT,
    key: "payment",
    label: "Thanh toán",
    icon: "payments",
  },
  {
    value: SUPPORT_CATEGORY.TECHNICAL,
    key: "technical",
    label: "Kỹ thuật",
    icon: "construction",
  },
  {
    value: SUPPORT_CATEGORY.OTHER,
    key: "other",
    label: "Khác",
    icon: "support_agent",
  },
...
function normalizeEnumValue(value, options, fallback) {
  ...
  const found = options.find(
    (item) => item.key === text || item.label.toLowerCase() === text,
  );
...
function isAdminMessage(message) {
  const role = String(
    message?.senderRole || message?.role || message?.senderType || "",
  ).toLowerCase();
  return (
    role.includes("admin") || Boolean(message?.isAdmin || message?.fromAdmin)
  );
}
...
export function SupportCenterView({ audience = "customer" }) {
      ...
      try {
        ...
        message.error(
          error.response?.data?.message ||
            error.message ||
            "Không thể tải trung tâm hỗ trợ.",
        );
      }
    },
  ...
  const selectedStatus = getOption(
    statusOptions,
    selectedTicket?.status,
    SUPPORT_STATUS.OPEN,
  );
  const selectedCategory = getOption(
    categoryOptions,
    selectedTicket?.category,
    SUPPORT_CATEGORY.OTHER,
  );
  const selectedPriority = getOption(
    priorityOptions,
  ...
  const summary = useMemo(() => {
    const activeCount = tickets.filter((ticket) => {
      ...
      return (
        value === SUPPORT_STATUS.OPEN || value === SUPPORT_STATUS.IN_PROGRESS
      );
    }).length;
    const resolvedCount = tickets.filter((ticket) => {
      ...
      return (
        value === SUPPORT_STATUS.RESOLVED || value === SUPPORT_STATUS.CLOSED
      );
    }).length;
  ...
  const handleOpenTicket = async (ticket) => {
    ...
    try {
      ...
      message.error(
        error.response?.data?.message ||
          error.message ||
          "Không thể tải hội thoại hỗ trợ.",
      );
    }
  };
  ...
  return (
    <div className="mx-auto max-w-[1180px] py-0 font-montserrat">
      <section className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="m-0 text-xs font-bold uppercase tracking-[0.16em] text-[#FF8228]">
            Help Center
          </p>
          <h1 className="m-0 mt-2 text-3xl font-black text-[#1b1c1c]">
            Trung tâm hỗ trợ
          </h1>
          <p className="m-0 mt-2 max-w-2xl text-sm text-[#555555]">
            Gửi ticket, theo dõi phản hồi từ admin và trao đổi trực tiếp trong
            cùng một luồng hỗ trợ.
          </p>
        </div>
        <Button
          type="primary"
      ...
      <section className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="rounded-2xl border-[#F1D5CD]">
          ...
          <p className="m-0 mt-2 text-3xl font-black text-[#1b1c1c]">
            {meta.totalCount}
          </p>
        </Card>
        <Card className="rounded-2xl border-[#B9EBFF] bg-[#F3FCFF]">
          ...
          <p className="m-0 mt-2 text-3xl font-black text-[#009BD6]">
            {summary.activeCount}
          </p>
        </Card>
        <Card className="rounded-2xl border-[#BDEFD2] bg-[#F5FFF9]">
          ...
          <p className="m-0 mt-2 text-3xl font-black text-[#23A455]">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[420px_minmax(0,1fr)]">
        <Card className="support-ticket-list-card rounded-2xl">
          <div className="mb-4">
            <h2 className="m-0 text-lg font-black text-[#1b1c1c]">
              Ticket của tôi
            </h2>
            <p className="m-0 mt-1 text-xs font-medium text-[#818A91]">
              {isWorker
                ? "Các vấn đề liên quan công việc, ví và hồ sơ thợ."
                : "Các vấn đề liên quan đặt lịch, thanh toán và tài khoản."}
            </p>
          </div>
          <form
            className="support-ticket-search mb-4"
          ...
          >
            ...
            <button
              type="submit"
              className="support-ticket-search-button"
            ...
            <Spin spinning={loading}>
              <div className="space-y-3">
                {tickets.map((ticket) => {
                  ...
                  return (
                    ...
                    >
                      <div className="mb-3 flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="m-0 truncate text-sm font-black text-[#1b1c1c]">
                              {getSubject(ticket)}
                            </p>
                      ...
                      <p className="m-0 line-clamp-2 text-xs leading-5 text-[#555555]">
                ...
                {!tickets.length && !loading && (
                  <Empty description="Chưa có ticket hỗ trợ" />
                )}
              </div>
            </Spin>
          </div>
          ...
          <div className="support-ticket-list-pagination flex justify-center">
            <Pagination
              ...
              onChange={(PageNumber, PageSize) =>
                setFilters((current) => ({ ...current, PageNumber, PageSize }))
              }
              size="small"
            />
          </div>
              ...
              <div className="border-b border-[#E8E8E8] pb-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h2 className="m-0 mt-1 text-2xl font-black text-[#1b1c1c]">
                      {getSubject(selectedTicket)}
                    </h2>
                    <p className="m-0 mt-2 text-sm leading-6 text-[#555555]">
                      {selectedTicket.description || "Chưa có mô tả chi tiết."}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <span
                      className={`rounded-full border px-3 py-1.5 text-xs font-bold ${selectedStatus.className}`}
                    >
                      {selectedStatus.label}
                    </span>
                    <Tag
                      color={selectedPriority.color}
                      className="!m-0 !rounded-full !px-3 !py-1.5 !text-xs !font-bold"
                    >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {selectedTicket.bookingId && (
                    <Tag className="!m-0 !rounded-full !px-3 !py-1.5">
                      Booking #
                      {String(selectedTicket.bookingId)
                        .slice(0, 8)
                        .toUpperCase()}
                    </Tag>
                  )}
                  ...
                  <div className="max-h-[420px] space-y-3 overflow-y-auto pr-2">
                    {sortedMessages.map((item, index) => {
                      ...
                      return (
                        ...
                        >
                          <Avatar
                            className={`!inline-flex !items-center !justify-center !font-bold ${admin ? "!bg-[#E8F8FF] !text-[#009BD6]" : "!bg-[#FFF0E6] !text-[#FF8228]"}`}
                          >
                            {getInitials(author, admin ? "AD" : "BT")}
                          </Avatar>
                          <div
                            className={`max-w-[82%] rounded-2xl border px-4 py-3 ${admin ? "border-[#B9EBFF] bg-[#F3FCFF]" : "border-[#F1D5CD] bg-[#FFF8F5]"}`}
                          >
                            <div className="mb-1 flex flex-wrap items-center gap-2">
                              <p className="m-0 text-sm font-bold text-[#1b1c1c]">
                                {author}
                              </p>
                              <span className="text-[11px] text-[#818A91]">
                                {formatFullDateTime(getMessageTime(item), "")}
                              </span>
                            </div>
                            <p className="m-0 whitespace-pre-wrap text-sm leading-6 text-[#4A4A4A]">
      ...
      <SupportTicketModal
        ...
        contextLabel={
          isWorker
            ? "Gửi ticket cho admin về công việc, hồ sơ hoặc thanh toán."
            : "Gửi ticket cho admin về đặt lịch, tài khoản hoặc thanh toán."
        }
