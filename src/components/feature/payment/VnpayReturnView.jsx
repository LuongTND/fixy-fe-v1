...
function DetailRow({ label, value }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-[#DEC0B1]/30 py-2 last:border-b-0">
      ...
      <span className="break-all text-right text-xs font-bold text-[#1b1c1c]">
        {value}
      </span>
    </div>
  );
    ...
    <nav className="fixed bottom-0 z-50 w-full rounded-t-xl border-t border-[#DEC0B1] bg-white shadow-lg md:hidden">
      <div className="flex w-full items-center justify-around px-4 py-3">
        {[
          ...
          >
            <span
              className={`material-symbols-outlined ${item.active ? "material-symbols-filled" : ""}`}
            >
              {item.icon}
            </span>
...
export function VnpayReturnView() {
  ...
  const message = isSuccess
    ? "Giao dịch đã được xác nhận thành công. Cảm ơn bạn đã sử dụng dịch vụ của Fixy."
    : verificationError ||
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FBF9F8] pb-20 md:pb-0">
      <main className="flex flex-1 items-center justify-center">
        <section
          className={`w-full max-w-[540px] overflow-hidden border bg-white shadow-[0px_2px_8px_rgba(0,0,0,0.08)] ${isSuccess ? "rounded-3xl border-[#DDDDDD]" : "rounded-xl border-[#DDDDDD]"}`}
        >
          <div className="flex flex-col items-center p-10 text-center">
            <div
              className={`mb-6 flex h-20 w-20 items-center justify-center rounded-full ${isSuccess ? "bg-[#39B54A1A]" : "bg-[#EA43351A]"}`}
            >
              <span
            ...
            <h1
              className={`${isSuccess ? "text-2xl" : "text-3xl"} mb-3 font-bold text-[#1b1c1c]`}
            >
              {isSuccess ? "Giao dịch thành công" : "Giao dịch thất bại"}
            </h1>
            <p
              className={`${isSuccess ? "text-sm text-[#4A4A4A]" : "text-sm text-[#555555]"} mb-8 max-w-[380px] leading-6`}
            >
              {message}
            </p>

            <div
              className={`mb-8 flex w-full flex-col ${isSuccess ? "gap-3 rounded-xl border border-[#DDDDDD]/50 bg-[#F5F5F5]/50 p-5" : "rounded-lg border border-[#DDDDDD] bg-[#FBF9F8] p-4"}`}
            >
              {amount && (
                <DetailRow label="Số tiền" value={formatVnpayAmount(amount)} />
              )}
            ...
            <div className="flex w-full flex-col gap-3">
              ...
              <BackNavigationButton className="inline-flex w-full items-center justify-center gap-2 rounded-full border-2 border-[#FF8228] bg-[#FF8228] px-6 py-3.5 text-center text-sm font-bold text-white shadow-sm transition-all hover:bg-[#F97316] active:scale-95">
                <span className="material-symbols-outlined text-[18px] leading-none">
                  arrow_back
                </span>
                Quay lại
          ...
          {isSuccess ? (
            <div className="flex items-center justify-center gap-2 border-t border-[#DEC0B1]/30 px-10 py-4">
              <span className="material-symbols-outlined text-[18px] text-[#555555]">
                verified_user
              </span>
            ...
            <div className="border-t border-[#DDDDDD] bg-[#FBF9F8] px-10 py-4 text-center">
              <p className="text-xs font-semibold text-[#555555]">
                ...
                <span className="font-semibold text-[#FF8228]">
                  Hotline 1900 xxxx
                </span>{" "}
                để được hỗ trợ.
              </p>
