...
export function StatusPending({ profile }) {
  ...
  const idImages =
    profile?.identificationImages || profile?.identificateImages || [];
  const certificates = profile?.certificates || [];
  const portfolioImages =
    profile?.profolioImages || profile?.portfolioImages || [];
    ...
    <div className="mx-auto max-w-[1200px] pb-10 pt-6">
      <section className="mb-6 rounded-xl border border-border-light bg-surface-bg p-6 shadow-sm">
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="m-0 text-xs font-bold uppercase tracking-[0.16em] text-secondary-container">
                Hồ sơ đang chờ duyệt
              </p>
              <h1 className="m-0 mt-1 text-2xl font-bold text-text-secondary">
                Fixy đang kiểm tra hồ sơ của bạn
              </h1>
              <p className="m-0 mt-2 max-w-3xl text-sm leading-6 text-text-tertiary">
                Hồ sơ đã được gửi thành công. Đội ngũ kiểm duyệt sẽ kiểm tra
                giấy tờ, dịch vụ và hình ảnh trước khi mở nhận việc.
              </p>
            </div>
          </div>
          <div className="rounded-lg border border-outline-variant/40 bg-background px-4 py-3">
            ...
            <p className="m-0 text-base font-bold text-text-secondary">
              24 - 48 giờ làm việc
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <section className="rounded-xl border border-border-light bg-surface-bg p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <Info
                ...
                value={`${profile?.maxDistanceKm || 0} km`}
              />
              <Info
                label="Ngày cấp CCCD"
                value={formatDate(profile?.citizenIdIssueDate)}
              />
            </div>
            {profile?.bio && (
              <p className="mt-5 rounded-lg bg-background p-4 text-sm leading-6 text-text-secondary">
                {profile.bio}
              </p>
            )}
          </section>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {services.map((service) => (
                <div
                  ...
                  className="rounded-lg border border-border-light bg-background p-4"
                >
        ...
        <aside className="space-y-6 lg:col-span-4">
          <DocumentCount
            label="Ảnh CCCD"
            count={idImages.length}
            expected="Tối thiểu 2 ảnh"
          />
          <DocumentCount
            label="Chứng chỉ"
          ...
          <DocumentCount
            ...
            expected="Ảnh công việc thực tế"
          />
          ...
          <section className="rounded-xl border border-secondary-container/30 bg-secondary-container/10 p-5">
            ...
            <p className="m-0 mt-3 text-sm leading-6 text-text-tertiary">
              Nếu thông tin bị thiếu hoặc ảnh không rõ, admin có thể từ chối hồ
              sơ và yêu cầu bạn cập nhật lại.
            </p>
          </section>
        </aside>
      </div>
  ...
  return (
    <div className="rounded-xl border border-border-light bg-surface-bg p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="m-0 text-sm text-text-tertiary">{label}</p>
          <p className="m-0 mt-1 text-xl font-bold text-text-secondary">
            {count}
          </p>
        </div>
        <span
          className={`material-symbols-outlined ${count ? "text-success" : "text-text-tertiary"}`}
        >
          {count ? "check_circle" : "draft"}
        </span>
      </div>
