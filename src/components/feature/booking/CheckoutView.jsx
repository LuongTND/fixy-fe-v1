...
function getPriceRange(payload) {
  ...
  const maxPrice = Number(payload?.maxPrice ?? payload?.data?.maxPrice ?? minPrice);
  return { minPrice, maxPrice };
}

function sortDraftsNewestFirst(drafts) {
  return [...drafts].sort((first, second) => {
    const firstTime = new Date(first.updatedDate || first.createdDate || first.createdAt || first.scheduledAt || 0).getTime();
    const secondTime = new Date(second.updatedDate || second.createdDate || second.createdAt || second.scheduledAt || 0).getTime();
...
export function CheckoutView() {
  ...
  const [categoryPriceRange, setCategoryPriceRange] = useState({ minPrice: 0, maxPrice: 0 });
    ...
    async function loadCheckout() {
      ...
      try {
        ...
        if (!draftData) {
          ...
          const newestDraft = sortDraftsNewestFirst(getDraftItems(draftsResponse))[0] || null;
        ...
        if (Array.isArray(draftData?.mediaIds) && draftData.mediaIds.length > 0) {
          const mediaResults = await Promise.allSettled(draftData.mediaIds.map((mediaId) => mediaApi.getById(mediaId)));
        ...
        if (alive) {
          message.error(error.response?.data?.message || error.message || 'Không thể tải thông tin yêu cầu đặt lịch.');
        }
      } finally {
  ...
  const totalAmount = useMemo(() => {
    const base = draft?.estimatedPrice || draft?.totalAmount || draft?.amount || categoryPrice || 0;
  ...
  const estimatedPriceLabel = useMemo(() => {
    ...
    if (maxPrice && maxPrice !== minPrice) return `${formatCurrency(minPrice)} - ${formatCurrency(maxPrice)}`;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      message.success('Đã gửi yêu cầu đặt lịch thành công đến các kỹ thuật viên.');
      sessionStorage.removeItem('bookingDraftId');
      
      // Redirect to the booking details page of the newly confirmed booking
      router.push(`/bookings/${bookingId}`);
    } catch (error) {
      message.error(error.response?.data?.message || error.message || 'Không thể xác nhận đặt dịch vụ.');
    } finally {
      setConfirming(false);
    }
  };
    ...
    return (
      <div className="mx-auto max-w-[1000px] py-16 text-center font-montserrat">
        <div className="animate-pulse space-y-4">
          ...
          <p className="text-sm font-semibold text-[#818A91]">Đang tải thông tin đặt lịch...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-[1000px] py-6 font-montserrat">
      <div className="mb-8">
        <div className="mb-4 flex items-center gap-3">
          <Link href="/booking/create" className="flex h-9 w-9 items-center justify-center rounded-full border border-[#dec0b1]/20 bg-white shadow-sm transition-all hover:bg-[#F5F5F5] no-underline">
            <span className="material-symbols-outlined text-[20px] text-[#1b1c1c]">arrow_back</span>
          </Link>
          <h1 className="text-2xl font-extrabold tracking-tight text-[#1b1c1c] md:text-3xl">Xác nhận đặt lịch</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-12">
          ...
          <div className="rounded-2xl border border-[#dec0b1]/10 bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#FF8228]/10">
                <span className="material-symbols-outlined text-[20px] text-[#FF8228]">assignment</span>
              </div>
              <h2 className="text-lg font-bold text-[#1b1c1c]">Chi tiết yêu cầu sửa chữa</h2>
            </div>

            {!draft ? (
              <div className="rounded-2xl border border-dashed border-[#dec0b1]/40 bg-[#FBF9F8] p-6 text-sm font-semibold text-[#818A91]">
                Chưa có bản nháp đặt dịch vụ. Hãy tạo yêu cầu trước khi xác nhận.
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#818A91]">Dịch vụ cần sửa</label>
                  <p className="text-base font-extrabold text-[#1b1c1c]">{draft?.categoryName || draft?.category?.name || 'Yêu cầu dịch vụ'}</p>
                </div>
                <div>
                  <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#818A91]">Thời gian sửa chữa</label>
                  <p className="text-sm font-semibold text-[#4A4A4A]">{formatSchedule(draft?.scheduledAt)}</p>
                </div>
                <div className="md:col-span-2">
                  <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#818A91]">Địa chỉ nhận thợ</label>
                  <p className="text-sm font-semibold leading-relaxed text-[#4A4A4A]">{draft?.address || 'Chưa có địa chỉ'}</p>
                </div>
                <div className="md:col-span-2">
                  <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#818A91]">Mô tả chi tiết sự cố</label>
                  <p className="text-sm font-medium leading-relaxed text-[#4A4A4A] whitespace-pre-wrap">{draft?.description || 'Chưa có mô tả'}</p>
                </div>
                
                {requestMedia.length > 0 && (
                  <div className="md:col-span-2">
                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#818A91]">Hình ảnh hiện trạng</label>
        ...
        <aside className="h-fit space-y-6 lg:sticky lg:top-24 lg:col-span-4">
          <div className="rounded-2xl border border-[#dec0b1]/10 bg-white p-6 shadow-xl">
            <h3 className="mb-6 text-lg font-bold text-[#1b1c1c]">Tóm tắt yêu cầu</h3>
            ...
            <div className="space-y-3 border-b border-[#E8E8E8] pb-6 text-sm">
              <div className="flex justify-between text-[#4A4A4A]">
                ...
                <span className="font-bold text-[#FF8228] bg-orange-50 px-2 py-0.5 rounded-md">{estimatedPriceLabel}</span>
              </div>
            </div>
            ...
            <div className="my-4 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 leading-relaxed flex items-start gap-2">
              ...
              <span>Đây là khoảng giá ước lượng ban đầu. Chi phí cuối cùng sẽ được thợ thỏa thuận trực tiếp và thanh toán sau khi hai bên thống nhất.</span>
            </div>
