...
function TrackingGoongMap({ workerLocation, destination, workerInfo }) {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      const [configResponse, goongjs, styleResponse] = await Promise.all([
        ...
        ensureGoongAssets(),
        fetch("https://tiles.goong.io/assets/goong_map_web.json")
          .then((res) => res.json())
          .catch(() => null),
      ]);
      const config = await configResponse.json();

      if (cancelled || !config.maptilesKey || !goongjs || !containerRef.current)
        return;
      ...
      mapRef.current.on("error", (event) => {
        if (
          event?.error?.message?.includes('Source layer "trees" does not exist')
        )
          return;
      ...
      workerMarkerRef.current = new goongjs.Marker(
        createMarkerElement({
          variant: "worker",
          icon: "two_wheeler",
          label: workerInfo?.fullName
            ? workerInfo.fullName.split(" ").slice(-2).join(" ")
            : "Thợ",
    ...
    initMap().catch((error) =>
      console.warn("Failed to initialize Goong map:", error),
    );
  ...
  useEffect(() => {
    ...
    if (workerLocation && destination) {
      ...
      mapRef.current.fitBounds(
        ...
        { padding: 70, maxZoom: 15, duration: 700 },
      );
    } else if (workerLocation || destination) {
      mapRef.current.flyTo({
        center: workerLocation || destination,
        zoom: 15,
        essential: true,
      });
    }
  ...
  return (
    <div className="relative h-[260px] overflow-hidden bg-slate-100 md:h-[320px]">
      <div
        ref={containerRef}
        className="absolute inset-0 h-full w-full [&_.goongjs-canvas]:!h-full [&_.goongjs-canvas]:!w-full [&_.mapboxgl-canvas]:!h-full [&_.mapboxgl-canvas]:!w-full"
      />
    </div>
  );
}
...
export function BookingDetailsView({ bookingId }) {
  ...
  const handleOpenChat = () => {
    if (statusKey === "pending" || statusKey === "matching") {
      message.warning(
        "Vui lòng đợi kỹ thuật viên nhận lịch để có thể nhắn tin.",
      );
      return;
    }
    setChatOpen(true);
  ...
  const handleRespondProposal = async (accept) => {
    ...
    try {
      await bookingApi.respondProposal(bookingId, {
        accept,
        rejectReason: accept ? null : "Khách hàng không đồng ý đề xuất.",
      });
  ...
  const handleApplyVoucher = async (codeToApply) => {
    ...
    try {
      ...
      if (found) {
        if (!found.isEligible) {
          throw new Error(
            formatIneligibleReason(found.ineligibleReason) ||
        ...
        message.success(
          `Đã chọn voucher: ${code}. Chi tiết giảm giá hiển thị ở phần Chi phí.`,
        );
      } else {
        throw new Error(
          "Mã voucher không tồn tại hoặc không áp dụng cho đơn này.",
        );
      }
    } catch (error) {
      const errMsg =
    ...
    try {
      ...
      if (paymentMethod === "wallet") {
        ...
        const discount = selectedVoucher
          ? selectedVoucher.calculatedDiscount
          : 0;
        const total = Math.max(0, baseTotal - discount);

        if (wallet && wallet.balance < total) {
          message.warning(
            "Số dư ví không đủ. Vui lòng nạp thêm tiền hoặc chọn VNPay.",
          );
          setPaying(false);
          return;
        }
        ...
        const { paymentUrl } =
          await paymentApi.createBookingVnpayPayment(bookingId);
        if (paymentUrl) {
  ...
  const workerMapLocation = hasWorkerLocation
    ...
    : null;
  const destinationMapLocation =
    booking?.lat &&
    booking?.lng &&
    Number(booking.lat) !== 0 &&
    Number(booking.lng) !== 0
  ...
  const addressCompound = geocodedAddress?.compound
    ? [geocodedAddress.compound.commune, geocodedAddress.compound.province]
        .filter(Boolean)
        .join(", ")
    : "";
  const trackerProgressClass =
    statusKey === "completed"
      ? "w-[80%]"
  ...
  const trackerSteps = [
    {
      ...
      completed: ["traveling", "arrived", "inprogress", "completed"].includes(
        statusKey,
      ),
    },
    {
      label: "Đang đến",
      icon: "moped",
      active: ["traveling", "arrived", "inprogress", "completed"].includes(
        statusKey,
      ),
      completed: ["arrived", "inprogress", "completed"].includes(statusKey),
    },
    {
      label: "Thực hiện",
      icon: "build",
      active: ["inprogress", "completed"].includes(statusKey),
      completed: ["completed"].includes(statusKey),
    },
    {
      label: "Hoàn thành",
      icon: "verified",
    ...
    return (
      <div className="mx-auto max-w-[800px] py-12 px-4 font-montserrat">
        <div className="rounded-2xl border-2 border-dashed border-[#dec0b1]/30 bg-white p-10 text-center shadow-sm">
          ...
          <p className="font-bold text-[#1b1c1c] text-lg mb-2">
            Không tìm thấy yêu cầu đặt lịch
          </p>
          <p className="text-sm text-[#818A91] mb-6">
            Yêu cầu này không tồn tại hoặc bạn không có quyền truy cập.
          </p>
          <Link
            href="/bookings"
            className="inline-flex items-center gap-2 rounded-xl bg-[#FF8228] px-6 py-2.5 font-bold text-white shadow-md hover:brightness-105 no-underline transition-all"
          >
    ...
    <div className="mx-auto max-w-[1200px] px-4 py-8 font-montserrat text-[#1b1c1c] min-h-screen">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            <span className="material-symbols-outlined text-[#1b1c1c]">
              arrow_back
            </span>
          </button>
          <div>
            ...
            <p className="text-xs font-semibold text-[#818A91] mt-0.5">
              Mã đặt lịch: #
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div
            className={`flex w-fit items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-bold uppercase tracking-wider ${statusMeta.color} shadow-sm`}
          >
            <span className="material-symbols-outlined text-[16px]">
              {statusMeta.icon}
            </span>
          ...
          <div className="relative flex justify-between items-center w-full max-w-[800px] mx-auto overflow-x-auto py-2">
            ...
            {trackerSteps.map((step, idx) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                    step.completed
                      ...
                      : step.active
                        ? "bg-[#FF8228] text-white ring-4 ring-[#FF8228]/20 shadow-md"
                ...
                <span
                  className={`text-[10px] font-bold uppercase tracking-wider mt-2.5 ${
                    step.completed
                      ? "text-[#39B54A]"
                      : step.active
                        ? "text-[#FF8228]"
        ...
        <section className="mb-8 overflow-hidden rounded-2xl border border-[#FF8228]/15 bg-white shadow-sm">
          <div className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="m-0 text-base font-extrabold text-[#1b1c1c]">
                  Theo dõi vị trí thợ
                </h2>
          ...
          <TrackingGoongMap
            workerLocation={workerMapLocation}
            destination={destinationMapLocation}
            workerInfo={tracking?.workerInfo}
          />
          ...
          <div className="grid grid-cols-1 border-t border-slate-100 md:grid-cols-3">
            <div className="border-b border-slate-100 p-5 md:border-b-0 md:border-r">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#818A91]">
                Kỹ thuật viên
              </span>
            ...
            <div className="border-b border-slate-100 p-5 md:border-b-0 md:border-r">
              ...
              <p className="mt-1 line-clamp-2 text-xs font-semibold leading-relaxed text-[#818A91]">
                {displayAddress}
              </p>
              {hasWorkerLocation && (
                <a
            ...
            <div className="p-5">
              ...
              <p className="mt-1 text-sm font-bold leading-relaxed text-[#1b1c1c]">
                {displayAddress}
              </p>
              {addressCompound && (
                <p className="mt-1 text-xs font-semibold text-[#818A91]">
        ...
        <div className="space-y-6 lg:col-span-8">
            ...
            <article className="rounded-2xl border border-[#FF8228]/20 bg-[#FF8228]/5 p-6 shadow-md animate-fade-in">
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-bold text-[#FF8228]">
                    Đề xuất mới từ thợ
                  </h3>
                  <p className="text-xs text-[#818A91] mt-0.5">
                    Kỹ thuật viên muốn thương lượng lại chi phí hoặc thời gian
                    đặt lịch.
                  </p>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 rounded-xl bg-white p-4 border border-[#dec0b1]/20">
                    ...
                    {booking.workerProposedNote && (
                      <div className="sm:col-span-2 pt-2 border-t border-slate-100">
                        ...
                        <p className="text-xs text-[#4A4A4A] mt-1 leading-relaxed italic">
                          &quot;{booking.workerProposedNote}&quot;
                        </p>
                      </div>
                    )}
          ...
          {statusKey === "pendingpayment" && (
            <article className="rounded-2xl border border-rose-100 bg-white p-6 shadow-md">
              ...
              <p className="text-xs text-[#818A91] mb-5">
                Vui lòng chọn phương thức thanh toán bên dưới để xác nhận công
                việc này.
              </p>
              ...
              <button
                ...
                onClick={() =>
                  openSupportTicket({
                    ...
                    contextLabel:
                      "Dành cho lỗi ví, voucher hoặc cổng thanh toán của booking này.",
                  })
                }
              ...
              <div className="mb-5">
                <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-[#818A91] mb-2">
                  <span className="material-symbols-outlined text-[15px] text-[#FF8228]">
                    local_activity
                  </span>
                ...
                {selectedVoucher ? (
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 border border-emerald-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-extrabold text-emerald-700 uppercase tracking-wider truncate">
                        {selectedVoucher.code}
                      </p>
                      {selectedVoucher.calculatedDiscount > 0 && (
                        <p className="text-[10px] text-emerald-600 mt-0.5">
                          Sẽ giảm{" "}
                          {formatCurrency(selectedVoucher.calculatedDiscount)}{" "}
                ...
                {!selectedVoucher && (
                  <div className="mt-2.5">
                    ...
                    >
                      ...
                      {loadingEligible
                        ? "Đang tải voucher..."
                        : `Chọn voucher từ kho (${eligibleVouchers.filter((v) => v.isEligible).length} khả dụng)`}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 mb-6">
                <button
                  ...
                  className={`flex items-center gap-3 rounded-xl border p-4 text-left transition-all ${
                    paymentMethod === "wallet"
                      ? "border-[#FF8228] bg-[#FF8228]/5 shadow-sm"
                      : "border-slate-200 hover:border-[#FF8228]/40"
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="block text-xs font-bold text-[#1b1c1c]">
                      Ví của tôi
                    </span>
                    <span className="block text-[10px] text-[#818A91] mt-0.5">
                      Số dư: {formatCurrency(wallet?.balance || 0)}
                    </span>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${paymentMethod === "wallet" ? "border-[#FF8228]" : "border-slate-300"}`}
                  >
                    {paymentMethod === "wallet" && (
                      <div className="w-2.5 h-2.5 rounded-full bg-[#FF8228]" />
                    )}
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div
                    className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${paymentMethod === "vnpay" ? "border-[#FF8228]" : "border-slate-300"}`}
                  >
                    {paymentMethod === "vnpay" && (
                      <div className="w-2.5 h-2.5 rounded-full bg-[#FF8228]" />
                    )}
              ...
              {paymentMethod === "wallet" &&
                ...
                wallet.balance < finalTotalAmount && (
                  <div className="mb-5 rounded-xl bg-rose-50 border border-rose-100 p-3 text-xs font-semibold text-rose-500 flex items-center gap-2">
                    ...
                    <span>
                      Ví không đủ số dư để thanh toán (
                      {formatCurrency(finalTotalAmount)}). Vui lòng nạp thêm
                      hoặc chọn VNPAY.
                    </span>
                  </div>
                )}
          ...
          <article className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-[#4A4A4A] whitespace-pre-line">
            ...
            <div className="grid grid-cols-1 gap-5 border-t border-slate-100 pt-5 sm:grid-cols-2">
              <div>
                ...
                <p className="font-bold text-sm text-[#1b1c1c] mt-0.5">
              ...
              <div>
                ...
                <p className="font-bold text-sm text-[#1b1c1c] mt-0.5 leading-relaxed">
                  {displayAddress}
                </p>
                {addressCompound && (
                  <p className="mt-1 text-xs font-semibold text-[#818A91]">
                  ...
                  <div className="grid grid-cols-3 gap-3 sm:grid-cols-5">
                    {booking.media.map((med) => (
                      <div
                        ...
                        className="group aspect-square overflow-hidden rounded-xl border border-slate-200 bg-[#F5F5F5]"
                      >
                        <AntImage
          ...
          {statusKey === "completed" && booking.media && (
            <article className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center gap-3">
                ...
                <h3 className="text-base font-bold text-[#39B54A]">
              ...
              <AntImage.PreviewGroup>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {booking.media
                    .filter(
                      (med) =>
                        med.category === MEDIA_CATEGORY.COMPLETION ||
                        med.category === 4,
                    )
                    .map((med) => (
                      <div
              ...
              {booking.media.filter(
                ...
                <p className="text-xs text-[#818A91] italic">
                  Không có ảnh nghiệm thu được lưu.
                </p>
              )}
            </article>
          )}
            ...
            <div className="space-y-3.5 text-sm">
              ...
              {isProposed && (
                <div className="flex justify-between items-center text-slate-400 text-xs">
                  ...
                  <span className="font-semibold text-[#FF8228] bg-orange-50 px-2 py-0.5 rounded-md">
                    {formatCurrency(booking.workerProposedPrice)}
                  </span>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                ...
                <span className="text-xl font-black text-[#FF8228]">
          ...
          {statusKey === "completed" && (
            <article className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center gap-3">
                ...
                <div>
                  <h2 className="m-0 text-sm font-extrabold text-[#1b1c1c]">
                    Đánh giá dịch vụ
                  </h2>
                  <p className="m-0 mt-0.5 text-xs text-[#818A91]">
                    Chia sẻ trải nghiệm sau khi hoàn thành.
                  </p>
                </div>
              </div>
                    ...
                    <div className="mb-3 flex items-start justify-between gap-3">
                      <div>
                        <p className="m-0 text-sm font-extrabold text-[#1b1c1c]">
                          {bookingReview.customer?.fullName ||
                            "Đánh giá của bạn"}
                        </p>
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          {bookingReview.images.map((image) => {
                            ...
                            return (
                              <div
                                key={image.id || url}
                                className="aspect-square overflow-hidden rounded-lg border border-white bg-slate-50"
                              >
                                <AntImage
                                  src={url}
                                  alt="Ảnh đánh giá"
                ...
                <>
                  <div className="mb-5 flex justify-center gap-1.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <span
                          className={`material-symbols-outlined text-[34px] ${(reviewHoverRating || reviewRating) >= star ? "text-[#FF8228]" : "text-slate-300"}`}
                        >
                          star
                        </span>
                      </button>
          ...
          <article className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            ...
            {technician ? (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100">
                  <button
                    ...
                    onClick={() =>
                      message.info(
                        "Tính năng gọi ẩn danh đang được phát triển.",
                      )
                    }
                  ...
                  <button
                    ...
                    title={
                      statusKey === "pending" || statusKey === "matching"
                        ? "Chờ kỹ thuật viên chấp nhận lịch để nhắn tin"
                        : ""
                    }
                  >
                    <span className="material-symbols-outlined text-lg">
              ...
              <div className="text-center py-6">
                ...
                <p className="text-xs font-semibold text-[#818A91]">
                  Đang tìm kiếm kỹ thuật viên phù hợp nhất...
                </p>
              </div>
            )}
          </article>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-sm leading-tight">
                    {technician?.fullName || "Hộp thoại thương lượng"}
                  </h3>
                  <p className="text-[10px] text-[#818A91] mt-0.5">
                    ID Đặt lịch #{bookingId.substring(0, 8).toUpperCase()}
                  </p>
                </div>
              </div>
      ...
      <Modal
        title={
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            <span className="material-symbols-outlined text-[#FF8228] text-[22px]">
              local_activity
            </span>
            <span className="text-base font-black text-[#1b1c1c]">
      ...
      >
        <div className="py-2">
          <p className="text-xs text-[#818A91] mb-4">
            Danh sách các mã giảm giá áp dụng cho đơn dịch vụ này của bạn.
          </p>
          ...
          {loadingEligible ? (
            ...
            <div className="py-8 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50">
              ...
              <p className="mt-2 text-xs font-bold text-slate-400">
                Không tìm thấy voucher khả dụng.
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!v.isEligible && v.ineligibleReason && (
                          <div className="mt-3 rounded-lg bg-rose-50 px-2.5 py-1.5 text-[10px] font-semibold text-rose-500 flex items-start gap-1">
                            ...
                            <span>
                              {formatIneligibleReason(v.ineligibleReason)}
                            </span>
                          </div>
                        )}
                      </div>
